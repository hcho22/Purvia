...
- **Score gates roll; operational gates stay fixed.** Operational gates require 1.0 effective coverage and zero provider errors: a failed metric or provider call must be non-green and must never quietly redefine "normal." Score gates (the metric values themselves) use a *rolling 4-week median*: a real, sustained quality improvement *should* rebaseline, so it is not later mistaken for a regression. Adapting to degradation is the failure mode to avoid; adapting to genuine improvement is correct — hence the split.
