...
- **The evaluator API is pinned.** RAGAS 0.4 moved the canonical implementations to `ragas.metrics.collections` and changed evaluator/embedding construction. The exact `0.4.3` pin keeps that contract reproducible; Answer Relevancy uses the explicit `text-embedding-3-small` model rather than a dependency default that can drift.
