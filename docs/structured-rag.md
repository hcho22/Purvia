...
python -m db_seed.crm_seed              # one-time seed
...
_Generated by `python -m evals.structured_rag.runner` at 2026-05-13T13:39:28 (eval ran in 66.3s)._

**Headline:** naive **10.0%** vs semantic **100.0%** — Δ **+90.0pp** on n=30 questions.

### Per-category accuracy

| Category | n | Naive | Semantic | Δ |
|---|---|---|---|---|
| join | 9 | 0.0% | 100.0% | +100.0pp |
| metric | 15 | 6.7% | 100.0% | +93.3pp |
| time | 6 | 33.3% | 100.0% | +66.7pp |
...
Naive SQL:
```sql
SELECT SUM(total) FROM crm.orders WHERE status = 'completed' LIMIT 200
```
...
Naive SQL:
```sql
SELECT SUM(total) AS gross_revenue FROM crm.orders WHERE status = 'completed' LIMIT 200
```
Semantic plan: `{"metrics": ["gross_revenue"], "dimensions": [], "filters": [], "time_grain": null}`
