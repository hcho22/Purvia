...
- [x] `MODULE_10_BASELINE_RECALL_AT_5` in `evals/retrieval/runner.py` renamed to `NON_REGRESSION_BASELINE_RECALL_AT_5` and re-pinned to post-US-116 levels (`vector 0.875 / keyword 0.917 / hybrid 0.950`), with a provenance comment (re-pinned 2026-07-12, source: clean `--viewers all --include-e6` 60-q golden-set run vs local Supabase at the code-default `SEARCH_SIMILARITY_THRESHOLD=0.3` (`backend/retrieval.py` `DEFAULT_SIMILARITY_THRESHOLD`, also what CI uses), full_access × pre_filter recall@5).
