...
**Status:** ✅ Done (implemented on `feat/us-072-widget-keys`) - `widget_keys` is the public widget's key registry and a FOURTH trust boundary (the admin-management surface) beside the three in CLAUDE.md/AGENTS.md. `public_key` is NON-SECRET (embedded verbatim in the buyer's loader `<script>`, US-083; stored in the clear - nothing to hash, unlike US-071's `token_hash`) and grants no access by itself: it only NAMES which workspace's bot a widget speaks to. `backend/widget_keys.py` holds the pure primitives (`generate_public_key()` → `wk_pk_<urlsafe>`, `is_widget_public_key()` shape guard). Migration `supabase/migrations/20260624130000_widget_keys.sql` creates `widget_keys(id, workspace_id fk ON DELETE CASCADE, public_key text unique, label, allowed_origins text[] default '{}', revoked_at timestamptz NULL, created_by fk ON DELETE SET NULL, created_at)` plus a non-unique `widget_keys_workspace_id_idx`. `role='admin'` legitimately enters the RLS here - the ONE place ADR-0002 allows it, since issuing/rotating/revoking keys IS the administrative action - via SELECT/INSERT/UPDATE policies gating on the membership clause WITH `wm.role='admin'`; there is intentionally NO DELETE policy (rotate = issue-new + revoke-old; revoked keys retained for audit). Revocation is a one-way DB-enforced latch: the `before update` trigger `_widget_keys_revoke_guard` rejects clearing or moving an already-set `revoked_at` (so a direct PostgREST PATCH behind the column-unrestricted admin UPDATE policy cannot re-activate a key), while `NULL → timestamp` (revoke) and `NULL → NULL` (label/origin edits) stay allowed. Endpoints (`main.py`): admin-JWT-authed `POST /api/support/widget-keys` (issue; first issuance lazily provisions the US-069 bot via best-effort idempotent `_ensure_workspace_bot`), `GET /api/support/widget-keys` (list active+revoked for the US-090 `/support/settings` UI), `POST /api/support/widget-keys/{key_id}/revoke` (one-way flip of an active key only); plus the anonymous public `POST /widget/keys/resolve` reading under the service role (`_resolve_widget_key`, filtered `public_key=eq AND revoked_at=is.null`) and returning `{"active": true}` or 404 with no workspace topology leaked. Test: `python -m backend.test_us072_widget_keys`. Trust-model invariant recorded in AGENTS.md/CLAUDE.md.
...
**Acceptance Criteria:**
- [x] Migration creates `widget_keys(id, workspace_id, public_key text unique, label text, allowed_origins text[], revoked_at timestamptz NULL, created_by uuid, created_at)`; public_key is non-secret (embedded in client JS). *(`20260624130000_widget_keys.sql` creates exactly this schema - `workspace_id` FK `ON DELETE CASCADE`, `allowed_origins` defaulting to `'{}'`, `created_by` FK `ON DELETE SET NULL`, unique `public_key`. `backend/widget_keys.py:generate_public_key()` mints `wk_pk_<urlsafe>`, non-secret and stored in the clear.)*
- [x] Multiple keys per workspace allowed; rotate = issue-new + revoke-old (no auto-rotation). *(No unique constraint on `workspace_id` - only the non-unique `widget_keys_workspace_id_idx`; rotation is `POST /api/support/widget-keys` again followed by `…/revoke` of the old key, and the deny-by-default DELETE keeps the revoked key beside its replacement for audit.)*
- [x] Resolution validates `revoked_at IS NULL` before minting/creating anything; **revoking blocks NEW conversations but never terminates live ones** (the opaque per-conversation token is independent of the key once minted). *(`_resolve_widget_key` is a service-role read filtered `public_key=eq AND revoked_at=is.null`; a revoked/unknown key matches zero rows → `None` → 404, so `/widget/keys/resolve` mints nothing. The US-071 per-conversation token is independent of the key once minted, so a live chat survives revocation.)*
- [x] First key issuance triggers lazy bot provisioning (US-069). *(`_ensure_workspace_bot` calls `support_bot.provision_workspace_bot(workspace_id, http=…)` on every issuance - best-effort and idempotent, so the first issuance provisions and later ones return the same id; a provisioning failure or a build without the support-bot module is logged and never blocks issuance.)*
- [x] `widget_keys` RLS restricts admin reads/writes to admins of the owning workspace (managed in `/support/settings`, US-090). *(SELECT/INSERT/UPDATE policies gate on the ADR-0002 membership `EXISTS` clause WITH `wm.role='admin'` - the one place `role` belongs; no DELETE policy; the public resolution path bypasses RLS under the service role. The one-way `_widget_keys_revoke_guard` trigger backstops the column-unrestricted UPDATE policy.)*
- [x] Typecheck/lint passes and migration applies. *(Landed via the no-mistakes review pass on this branch - `070a1df`/`7cec0a9` reuse the service-role helper and map upstream 5xx→502 / 4xx→400/403/404 across all four endpoints, `1bfeae1` adds the DB-enforced one-way revoke latch. Migration + RLS + revoke latch exercised by `backend/test_us072_widget_keys.py`'s integration/security layer; the pure primitives by its always-on unit layer.)*
**Validation Test:**
