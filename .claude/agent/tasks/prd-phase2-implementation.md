...
## 4a. Implementation status (updated 2026-06-23)
...
| E — Support surface (US-067–093, beyond US-066) | 🟡 **In progress** | US-066 migration + US-067 (status-machine latch, PR #29), US-068 (JWT minting, PR #31), US-069 (bot provisioning, PR #32), US-070 (bot retrieval, PR #33), US-071 (opaque conversation token, PR #34), US-072 (widget-key registry + not-revoked resolution gate, PR #35), US-073 (per-key origin allowlist, PR #37), US-074 (public-widget CORS posture, local) and US-075 (swappable `RateLimiter` seam + Postgres counter migration, local) have landed; the rest of the conversation/widget runtime (customer SSE, first-message flow, admin surface) and US-076–093 remain. |
| F — Eval gate + golden-set genericization (US-101–112) | ⬜ Not started | The configurable E8 gate (comment-vs-fail) referenced by US-059 lives here; not yet built. |
...
- [ ] First escalating turn latches the whole conversation: after latch the bot never runs the ADR-0003 pipeline again for that conversation (asserted in US-080). *(NOT this story: the DB latch enabling it is delivered here, but the runtime "bot stops the ADR-0003 pipeline after latch" behaviour and its assertion are US-080's scope - left unchecked deliberately. See Status.)*
