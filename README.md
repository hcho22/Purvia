...
- **`.github/workflows/backend-unit-tests.yml`** — runs on every PR to `main` and executes `python -m backend.run_offline_tests`. The runner discovers every `backend/test_*.py` module by default, removes live-service credentials, supplies the current suite's deterministic tokenizer seam, forces hosted-model libraries offline, guards normal Python INET socket paths, and sets `PURVIA_OFFLINE_TESTS=1` so mixed unit/integration modules stop after their offline layer. This is a current-suite no-runtime-download or live-service-dependency contract, not an OS-level egress sandbox; arbitrary subprocess-level egress is outside its enforcement boundary. The small explicit exclusion map in `backend/run_offline_tests.py` contains only modules with no offline assertions and documents the local Supabase/Postgres or credentialed API they require. A newly added module is therefore tested automatically; a successful module must emit a `PASS:` or `OK:` verdict, so a live-only test that merely skips cannot silently turn green. Install the same focused dependencies with `pip install -r backend/requirements-test.txt`.
