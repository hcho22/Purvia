...
| [`docs/reranker-bakeoff.md`](docs/reranker-bakeoff.md) | Eval-only bake-off (US-117) of each reranker backend on top of hybrid retrieval — the runner's `--reranker {none,llm,cohere,voyage}` flag reusing the production rerank path, the per-category MRR / nDCG@5 / recall table, latency, and why the recommendation is to keep `RERANKER=none`. |
...
- **`.github/workflows/retrieval-eval-nightly.yml`** — daily 02:00 UTC. Publishes snapshots to `docs/nightly/<DATE>.md` + `.json`, plus a `hybrid+llm` reranker observability run (`docs/nightly/<DATE>-rerank-llm.json`, US-117) in a separate `continue-on-error` step so a flaky reranker run never blocks baseline publication. See [`docs/reranker-bakeoff.md`](docs/reranker-bakeoff.md).
...
python -m evals.retrieval.runner --reranker llm       # eval-only reranker bake-off on top of retrieval (none/llm/cohere/voyage; none is a pass-through). See docs/reranker-bakeoff.md
