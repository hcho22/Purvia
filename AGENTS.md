...
- **Lifetime ~24h**, slid forward on every successful resume while not resolved; refresh extends `expires_at` on the *same* token (the raw token is stable across reloads - it is "returned once", never rotated).
- Endpoints (`main.py`): `POST /widget/conversations/resume` and `GET /widget/conversations/{id}/transcript`, both authed by the opaque token. The transcript endpoint enforces the binding (resumed conversation id must equal the path id) so "token-for-X requesting Y" is rejected. `_issue_conversation_token(http, conversation_id)` is the issuance helper US-078 will call on first message. Public-widget CORS is US-074's concern; these routes do not widen the `/api/*` `FRONTEND_ORIGINS` posture.
